...
//    ui->label_axel4->setVisible(showImmature || showWatchOnlyImmature);
...
//        model->getWatchBalance(), model->getWatchUnconfirmedBalance(), model->getWatchImmatureBalance());
        ...
        connect(ui->blabel_axel, SIGNAL(clicked()), this, SLOT(openMyAddresses()));

        emit model->makeBalance();
    }

    // update the display unit, to not use the default ("axel")
    updateDisplayUnit();
}
...
void OverviewPage::updateBlockChainInfo()
{
    if (masternodeSync.IsBlockchainSynced())
    {
        ...
        double BlockReward = GetBlockValue(CurrentBlock);
        double BlockRewardaxel =  static_cast<double>(BlockReward/COIN);
...
void OverviewPage::on_pushButton_Discord_clicked() {
    QDesktopServices::openUrl(QUrl("https://discord.gg/7tNXcGg", QUrl::TolerantMode));
}
void OverviewPage::on_pushButton_Telegram_clicked() {
    QDesktopServices::openUrl(QUrl("https://t.me/axelnetwork", QUrl::TolerantMode));
}
