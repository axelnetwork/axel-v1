...
#if defined(NDEBUG)
#error "axel cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uaxel) are considered zero fee (for relaying and mining)
...
bool CheckTransaction(const CTransaction& tx, CValidationState& state)
{
    ...
    // Size limits
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION) > AXEL_MAX_BLOCK_SIZE)
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
...

    {
        ...
        if (nSigOps > AXEL_MAX_TX_SIGOPS)
            return state.DoS(0,
                error("AcceptToMemoryPool : too many sigops %s, %d > %d",
                    hash.ToString(), nSigOps, AXEL_MAX_TX_SIGOPS),
...
CAmount GetBlockValue(int nHeight)
{
    ...
    else if (nHeight >= 500 && nHeight < 411840) {
        nSubsidy = 15 * COIN;
    }
    else {
        nSubsidy = 61 * COIN;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (nSigOps > AXEL_MAX_BLOCK_SIGOPS)
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    // Size limits
    if (block.vtx.empty() || block.vtx.size() > AXEL_MAX_BLOCK_SIZE || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION) > AXEL_MAX_BLOCK_SIZE)
...
int nLoaded = 0;
    try {
        ...
        CBufferedFile blkdat(fileIn, 2 * AXEL_MAX_BLOCK_SIZE, AXEL_MAX_BLOCK_SIZE + 8, SER_DISK, CLIENT_VERSION);
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // read size
                blkdat >> nSize;
                if (nSize < 80 || nSize > AXEL_MAX_BLOCK_SIZE)
                    continue;
