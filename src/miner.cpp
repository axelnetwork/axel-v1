...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        while (!vecPriority.empty()) {
            ...
            if (nBlockSigOps + nTxSigOps >= AXEL_MAX_BLOCK_SIGOPS)
                continue;
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("axelMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("axelMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("axelMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("axel-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        //LogPrintf("Running axelMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(), ::GetSerializeSize(*pblock, SER_NETWORK, PROTOCOL_VERSION));
